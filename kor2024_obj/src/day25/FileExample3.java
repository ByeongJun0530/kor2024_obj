...
public class FileExample3 {
    public static void main(String[] args) {
        try {
            // 생각해보기 :
            // 1. - 키보드로부터 입력받은 문자열을 day25 폴더 내 test2.txt 에 저장하시오.
            // [1] 입력객체
            Scanner scan = new Scanner(System.in);
            // [2] 키보드로 입력받기
            System.out.print("문자열을 입력하시오 : ");
            String outStr = scan.next();
            // [3] 파일출력 객체 샏성, 파일의 경로 지정
            ...
            // 현재 읽어올 파일의 용량을 몰라서 임의로 1000개 한다.

            // [2-2] 읽어올 파일의 용량/바이트 개수를 확인한 후 파일의 용량만큼 배열 선언
            File file = new File("./src/day25/test2.txt");
            System.out.println(file.isFile()); // true
            System.out.println(file.getName()); // text2.txt
            ...
            // ! : 만역에 파일 객체가 지정한 경로에 파일이 존재하지 않으면 예외 발생
        } catch (FileNotFoundException e) {e.printStackTrace();}
        // ! : 만약에 파일 객체가 입/출력 중에 외부적인 요소로 문제가 발생하면 예외 발생
