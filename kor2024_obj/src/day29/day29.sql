...
create table board(
    ...
    -- bno 게시물번호 -> 숫자 -> 숫자 중에 적절한 타입 -> int
    -- unsigned -> 부호가 없다 -> int (+- 21억 정도) -> 총 42억 정도
    -- auto_increment -> 레코드/행/표에 값 삽입시 자동으로 번호가 순차적으로 자동 할당 해주는 키워드
    bcontent longtext not null,
    -- longtext	-> 게시물의 내용이 많은 내용들이 들어가면 좋을 것 같아서 4gb 선택
    -- not null -> 게시물 등록시 게시물 내용은 빈 내용이 들어가면 안될 것 같아서 차단, not null
    bwriter varchar(50) not null,
    -- varchar() -> 작성자는 적절하게 50글자까지 입력받을 수 있게 선택
    bpwd int not null,
    -- 비밀번호는 숫자 타입으로 int 사용,
    primary key(bno)
    -- primary key 이란? 기본(식별)키, 여러개의 필드 중에서 식별 가능한 필드, 중복X
    -- 게시물번호를 pk로 사용하는 이유? 게시물 내용/작성자/비밀번호는 충분히 중복 데이터가 존재할 경우가 크다.
    -- 관례적(무조건x 일반적o)으로 테이블마다 1개 이상의 pk(기본키) 필수,
    -- (주민등록번호o/사람이름x)/(학번o/학생이름x)/(제품번호o/제품이름x)/(isbn o/도서명x)
);
...
select* from board; # 여러번 실행 가능.
select bcontent, bwriter from board;		-- 지정한 테이블 'bcontent'와 'bwriter' 필드의 레코드 조회/검색

# (2) insert : 레코드 삽입/추가 SQL
# 1. insert into 테이블명 values(값1, 값2, 값3);
	# 전체 필드에 값을 삽입시 삽입할 필드명을 생략해도 된다.
# 2. insert into 테이블명 ( [값1]필드명1, [값2]필드명2, [값3]필드명3 ) values ( 값1, 값2, 값3 );
	# 특정 필드의 값만 삽입시 삽입할 필드명을 명시 해야한다.
insert into board values(1, '안녕하세요', '유재석', 1234); # 1번 실행 # board 테이블에 필드선언 순서대로 'bno' 에는 1, 'bcontent'에는 안녕하세요, 'bwriter'에는 유재석, 'bpwd'에는 1234 대입
-- insert into board value('그래 안녕', '강호동', 4567); # 오류 발생 # 필드의 개수와 데이터 개수가 일치하지 않다.
...
#		update 테이블명 set 수정할필드명 = 새로운 값 where 조건필드 = 값
update board set bwriter = '하하' where bno = 1; # 1번 실행
-- update board set bwriter = '하하'; # -- 레코드 조건이 없으면 전체수정

# (4) delete : 특정 레코드 (한 줄) 삭제 SQL
# 1.지정한 테이블의 전체 레코드 삭제
#		delete from 테이블명;
# 2.지정한 테이블의 특정 조건을 충족하는 레코드만 삭제
#		delete from 테이블명 where 조건필드 = 조건값
delete from board where bno = 1; -- 만약에 게시물 번호가 1번이면 레코드를 삭제한다.

# [실습]
/*
[문제1] : product 테이블에 새로운 제품을 추가하세요. 제품의 정보는 다음과 같습니다, 테이블 존재 가정 하에
	필드명 : 값
	product_id : 201
...
insert into product ( product_id, product_name, category, price, stock_quantity)
	values ( 201, 'Wireless mouse', 'Electonics', 29.99, 150 );
...
# [레코드 조회]
#(1.특정 필드 조회) select 필드명1, 필드명2 from 테이블명,
#(2.전체 필드 조회) select* from 테이블명,
#(3.조건 레코드 조회) select* from 테이블명 where 조건필드명 = 조건값
...
# [필드값 업데이트] update 테이블명 set 수정할필드명 = 새로운값 where 조건필드 = 조건값;
update employee set email = 'john.doe@company.com' where employee_id = 101;
