...
public class BoardDto {
    ...
    // 왜? 결국에는 DB에 저장된 데이터를 VIEW(html/css/js)까지 이동하는 객체
    // [1] 필드
    ...
    // DTO 주로 DAO 에서 또는 VIEW 에서 객체 생성시 개발자(본인) 초기화할 필드로 생성자를 구성한다. (정답 X) -> 끝 X -> 빌드패턴
        // - 자동완성 생성자 만들기 : 오른쪽 클릭
    ...
    // [3] 메소드, DTO : 실질적인 기능 역할 x , 단순 이동 객체이므로
    // DTO 주로 Getter/Setter => 필드가 잠금(private)이라서 외부 클래스 간접 접근 허용 , toString(객체의 필드 확인용) 역할이 크다.
        // - 자동완성 getter/setter : 우클릭
